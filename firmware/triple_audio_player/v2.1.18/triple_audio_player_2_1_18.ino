...
void setup() {
  ...
  if (settings.wifiSsid[0] == '\0') {
    if (DeviceWifiProvisioning::provision(settings.wifiSsid, sizeof(settings.wifiSsid),
...
void loop() {
  ...
  if (mqtt.connected()) {
    mqtt.loop();
    if (!stationStateTopic.isEmpty() && millis() - lastStationStateMs >= STATION_STATE_INTERVAL_MS) publishStationState();
  }
