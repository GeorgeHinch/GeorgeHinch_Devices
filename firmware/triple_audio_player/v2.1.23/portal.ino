...
void sendConfigJson() {
  ...
  doc["begButtonEnabled"] = settings.begButtonAction != BEG_DISABLED;
  doc["begSyncPlayer"] = settings.begSyncPlayer;
  doc["begButtonMode"] = modeToString(settings.begButtonMode);
  doc["begButtonVolume"] = settings.begButtonVolume;
  doc["begButtonTrack"] = settings.begButtonTrack;
  doc["begButtonFolder"] = settings.begButtonFolder;
...
void saveManifestConfiguration() {
  JsonDocument doc;
...
void savePortalSettings() {
  ...
  settings.begButtonAction = configServer.hasArg("begButtonEnabled")
      ? boundedArgument("begaction", BEG_PLAYER_1, BEG_PLAYER_1, BEG_PLAYER_3)
      : BEG_DISABLED;
  settings.begSyncPlayer = configServer.hasArg("begSyncPlayer");
  if (!settings.begSyncPlayer) {
    settings.begButtonMode = stringToMode(configServer.arg("begButtonMode").c_str());
    settings.begButtonVolume = boundedArgument("begButtonVolume", settings.begButtonVolume, 0, 30);
    settings.begButtonTrack = boundedArgument("begButtonTrack", settings.begButtonTrack, 1, 255);
    settings.begButtonFolder = boundedArgument("begButtonFolder", settings.begButtonFolder, 1, 99);
  }
  ...
  for (int i = 0; i < 3; i++) {
    ...
    players[i].muted = !configServer.hasArg("playerEnabled" + number);
    savePlayerSettings(i);
  }

  saveDeviceSettings();
  configServer.send(200, "text/html; charset=utf-8",
    "<!doctype html><meta name='viewport' content='width=device-width'><style>body{font-family:system-ui;padding:2rem;max-width:40rem;margin:auto}a{color:#7a43c4}</style><h1>Settings saved</h1><p>The Audio Player is restarting now.</p><p>If this page does not close, reconnect to its normal Wi-Fi network.</p>");
  delay(1000);
  ESP.restart();
}
...
void beginConfigPortal(bool startApNow) {
  ...
  if (!configServerStarted) {
    ...
    configServer.on("/api/configure", HTTP_POST, saveManifestConfiguration);
    configServer.on("/api/command", HTTP_POST, runManifestCommand);
