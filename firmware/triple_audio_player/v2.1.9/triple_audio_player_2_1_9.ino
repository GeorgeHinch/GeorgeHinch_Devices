...
#define DEFAULT_BEG_BUTTON_RUN_SECONDS 20
...
bool          begButtonRunActive = false;
int8_t        begButtonPlayerIndex = -1;
unsigned long begButtonStopAtMs = 0;
...
void loadDeviceSettings() {
  ...
  settings.begButtonRunSeconds = constrain(
      prefs.getUShort("begTime", DEFAULT_BEG_BUTTON_RUN_SECONDS), 1, 3600);
...
void saveDeviceSettings() {
  ...
  prefs.putUShort("begTime", settings.begButtonRunSeconds);
...
// ════════════════════════════════════════════════════════════════
//  BUTTON — starts the portal-selected player for the configured run time
// ════════════════════════════════════════════════════════════════
  ...
  if ((millis() - lastDebounceTime) > DEBOUNCE_MS) {
    ...
    if (reading == LOW && debouncedButton == HIGH) {
      ...
      if (!dfpPresent[playerIndex]) {
        ...
        Serial.printf("Button: triggering P%d for %u seconds\n", playerIndex + 1,
                      settings.begButtonRunSeconds);
        p.playRequested = true;
        begButtonRunActive = true;
        begButtonPlayerIndex = playerIndex;
        begButtonStopAtMs = millis() + static_cast<unsigned long>(settings.begButtonRunSeconds) * 1000UL;
    }
    debouncedButton = reading;
  }
}
...
void serviceBegButtonTimer() {
  if (!begButtonRunActive || static_cast<long>(millis() - begButtonStopAtMs) < 0) return;
  if (begButtonPlayerIndex >= 0 && begButtonPlayerIndex < 3) {
    players[begButtonPlayerIndex].stopRequested = true;
  }
  begButtonRunActive = false;
  begButtonPlayerIndex = -1;
}
