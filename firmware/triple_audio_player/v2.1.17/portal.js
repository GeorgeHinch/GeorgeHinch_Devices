const PREVIEW_DEFAULTS = {
  deviceId: 'AUDIOPLAY_123456789ABC', ip: '192.168.4.1', firmware: '2.1.16', connected: true,
  globalBegEnabled: true, begButtonEnabled: true, begAction: 1, begSyncPlayer: true,
  begButtonMode: 'LOOP_TRACK', begButtonVolume: 15, begButtonTrack: 1, begButtonFolder: 1,
  players: [
  ...
  mqttUser: 'layout-controller', jmriChannel: '/trains/track/', otaState: 'Up to date (v2.1.15)'
};
...
function updatePlayerUi(index) {
  ...
  const player = currentConfig.players[index];
  const mode = document.querySelector(`#mode${number}`).value;
  const available = player.present;
  const enabled = document.querySelector(`#playerEnabled${number}`).checked;
  document.querySelector(`#player${number}-settings`).hidden = !available || !enabled;
  document.querySelector(`#track${number}-field`).hidden = mode === 'LOOP_FOLDER';
  document.querySelector(`#folder${number}-field`).hidden = mode !== 'LOOP_FOLDER';
  [`mode${number}`, `volume${number}`, `track${number}`, `folder${number}`, `playerEnabled${number}`].forEach((id) => {
    document.querySelector(`#${id}`).disabled = !available;
  });
  document.querySelector(`#track${number}`).disabled = !available || mode === 'LOOP_FOLDER';
  document.querySelector(`#folder${number}`).disabled = !available || mode !== 'LOOP_FOLDER';
}

function updateExternalButtonUi() {
  const enabled = document.querySelector('#begButtonEnabled').checked;
  const syncPlayer = document.querySelector('#begSyncPlayer').checked;
  const customMode = document.querySelector('#begButtonMode').value;
  document.querySelector('#external-button-settings').hidden = !enabled;
  document.querySelector('#external-button-custom-settings').hidden = !enabled || syncPlayer;
  document.querySelector('#begaction').disabled = !enabled;
  document.querySelector('#begSyncPlayer').disabled = !enabled;
  document.querySelector('#begButtonMode').disabled = !enabled || syncPlayer;
  document.querySelector('#begButtonVolume').disabled = !enabled || syncPlayer;
  document.querySelector('#begButtonTrack').disabled = !enabled || syncPlayer || customMode === 'LOOP_FOLDER';
  document.querySelector('#begButtonFolder').disabled = !enabled || syncPlayer || customMode !== 'LOOP_FOLDER';
  document.querySelector('#beg-button-track-field').hidden = !enabled || syncPlayer || customMode === 'LOOP_FOLDER';
  document.querySelector('#beg-button-folder-field').hidden = !enabled || syncPlayer || customMode !== 'LOOP_FOLDER';
}
...
function setPlayer(index, player) {
  ...
  setChecked(`playerEnabled${number}`, player.present && !player.muted);
  document.querySelector(`#player${number}-panel`).classList.toggle('is-unavailable', !player.present);
...
function populate(config) {
  ...
  config.players.forEach((player, index) => setPlayer(index, player));
  ...
  setChecked('begButtonEnabled', config.begButtonEnabled !== false && config.begAction !== 0);
  setValue('begaction', config.begAction || 1);
  setChecked('begSyncPlayer', config.begSyncPlayer !== false);
  setValue('begButtonMode', config.begButtonMode || 'LOOP_TRACK');
  setValue('begButtonVolume', config.begButtonVolume ?? 15);
  setValue('begButtonTrack', config.begButtonTrack || 1);
  setValue('begButtonFolder', config.begButtonFolder || 1);
...
document.querySelector('#begButtonEnabled').addEventListener('change', updateExternalButtonUi);
document.querySelector('#begSyncPlayer').addEventListener('change', updateExternalButtonUi);
document.querySelector('#begButtonMode').addEventListener('change', updateExternalButtonUi);
[1, 2, 3].forEach((number) => document.querySelector(`#mode${number}`).addEventListener('change', () => updatePlayerUi(number - 1)));
[1, 2, 3].forEach((number) => document.querySelector(`#playerEnabled${number}`).addEventListener('change', () => updatePlayerUi(number - 1)));
