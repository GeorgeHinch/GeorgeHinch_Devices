...
void startPlayback(int idx, PlayMode mode, uint8_t track, uint8_t folder) {
  PlayerState& p = players[idx];
...
void startPlayback(int idx) {
  ...
  startPlayback(idx, p.mode, p.track, p.folder);
}
  ...
  if ((millis() - lastDebounceTime) > DEBOUNCE_MS) {
    ...
    if (reading == LOW && debouncedButton == HIGH) {
      ...
      if (loopMode && (p.playing || p.playRequested)) {
        ...
        p.playRequested = false;
        p.needsRestart = false;
        startPlayback(playerIndex, buttonMode, buttonTrack, buttonFolder);
