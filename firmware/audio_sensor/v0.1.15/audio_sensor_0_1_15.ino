...
#include "HWCDC.h"
#define AUDIO_SENSOR_USES_HWCDC 1
#else
#define AUDIO_SENSOR_USES_HWCDC 0
#endif
...
constexpr uint32_t OTA_HTTP_TIMEOUT_MS = 5000;
constexpr uint32_t OTA_DOWNLOAD_IDLE_TIMEOUT_MS = 10000;
...
bool allOutputsTestActive = false;
...
#if AUDIO_SENSOR_USES_HWCDC
char usbCdcPendingCommand[SERIAL_SETUP_CMD_BUFFER + 1];
volatile uint8_t usbCdcPendingCommandLen = 0;
volatile bool usbCdcPendingCommandReady = false;
portMUX_TYPE usbCdcRxMux = portMUX_INITIALIZER_UNLOCKED;
#endif
...
void handleSerialLightsCommand(const String& cmd) {
  if (cmd == "LIGHTS" || cmd == "LIGHTS ON" || cmd == "TEST_LIGHTS") {
    // Use the normal crossing state machine, but suppress audio. This tests
    // the shift-register outputs and the configured alternating/strobe mode.
    startCrossing(true);
    Serial.printf("Light test running in %s mode.\n",
                  cfg.lightPattern == LIGHT_PATTERN_STROBE ? "strobe" : "alternating");
    return;
  }

  if (cmd == "LIGHTS OFF" || cmd == "STOP LIGHTS" || cmd == "STOP_LIGHTS") {
    stopCrossing();
    Serial.println("Light test stopped.");
  }
}

void handleSerialAlarmCommand(const String& cmd) {
  const bool allOutputs = (cmd == "ALARM ALL" || cmd == "ALARM ALL ON" ||
                           cmd == "TEST_ALARM_ALL");
  if (cmd == "ALARM" || cmd == "ALARM ON" || cmd == "CROSSING" ||
      cmd == "CROSSING ON" || cmd == "TEST_ALARM" || allOutputs) {
    // Run the full crossing path. ALARM ALL is a diagnostic mode that drives
    // all eight 74HC595 outputs together instead of only QA/QB.
    allOutputsTestActive = allOutputs;
    startCrossing(false);
    if (allOutputs) {
      Serial.printf("All-channel alarm test running in %s mode with track %u at volume %u.\n",
                    cfg.lightPattern == LIGHT_PATTERN_STROBE ? "strobe" : "alternating",
                    cfg.track, cfg.volume);
    } else {
      Serial.printf("Full alarm test running in %s mode with track %u at volume %u.\n",
                    cfg.lightPattern == LIGHT_PATTERN_STROBE ? "strobe" : "alternating",
                    cfg.track, cfg.volume);
    }
    return;
  ...
  if (cmd == "ALARM OFF" || cmd == "CROSSING OFF" || cmd == "STOP ALARM" ||
      cmd == "STOP_ALARM" || cmd == "STOP CROSSING" || cmd == "STOP_CROSSING" ||
      cmd == "ALARM ALL OFF") {
    allOutputsTestActive = false;
    stopCrossing();
    Serial.println("Full alarm test stopped.");
  }
}
...
void writeShiftRegister(uint8_t srValue) {
...
bool downloadAndInstall(const String& url, const String& expectedSha256, size_t expectedSize) {
  if (url.isEmpty() || expectedSha256.length() != 64 || expectedSize == 0) {
    Serial.println("OTA: invalid update metadata; continuing normal operation");
    publishOtaState("ERROR: invalid update metadata");
    return false;
  }
  ...
  if (contentLength <= 0) {
    Serial.println("OTA: firmware response has no usable content length; continuing normal operation");
    publishOtaState("ERROR: missing firmware size");
    http.end();
    return false;
  }
  if (static_cast<size_t>(contentLength) != expectedSize) {
  ...
  while (written < static_cast<size_t>(contentLength)) {
    if (!http.connected() && !stream->available()) {
      Serial.println("OTA: firmware connection ended early; continuing normal operation");
      Update.abort();
      mbedtls_sha256_free(&sha);
      http.end();
      publishOtaState("ERROR: incomplete firmware");
      return false;
    }

    size_t available = stream->available();
    if (!available) {
      if (millis() - lastDataMs >= OTA_DOWNLOAD_IDLE_TIMEOUT_MS) {
        Serial.println("OTA: firmware download timed out; continuing normal operation");
        Update.abort();
        mbedtls_sha256_free(&sha);
        http.end();
        publishOtaState("ERROR: firmware download timeout");
        return false;
      }
      delay(1);
      continue;
    ...
    size_t remaining = static_cast<size_t>(contentLength) - written;
    size_t toRead = min(available, sizeof(buffer));
    if (toRead > remaining) toRead = remaining;
    size_t count = stream->readBytes(buffer, toRead);
    if (!count) continue;
...
void checkForOtaUpdate(bool force = false) {
  ...
  if (entry.isNull()) {
    Serial.printf("OTA: no manifest entry for %s; continuing normal operation\n", DEVICE_TYPE);
...
void serviceSetupPortal() {
  ...
  // Poll USB CDC directly so commands remain available while the application
  // is running in either portal or normal Wi-Fi mode.
...
void processSerialCommandLine(const String& input) {
  String rawCmd = input;
  rawCmd.trim();
  String cmd = rawCmd;
  cmd.toUpperCase();
  ...
  if (cmd == "SOUND" || cmd == "SOUNDS" || cmd == "SOUND ON" ||
      cmd == "PLAY SOUND" || cmd == "TEST_SOUND" || cmd == "TEST SOUND" ||
      cmd == "SOUND OFF" || cmd == "STOP SOUND" || cmd == "STOP_SOUND") {
    if (cmd == "SOUNDS" || cmd == "PLAY SOUND" || cmd == "TEST SOUND") {
      cmd = "SOUND";
    }
    handleSerialSoundCommand(cmd);
  } else if (cmd == "LIGHTS" || cmd == "LIGHTS ON" || cmd == "TEST_LIGHTS" ||
             cmd == "LIGHTS OFF" || cmd == "STOP LIGHTS" || cmd == "STOP_LIGHTS") {
    handleSerialLightsCommand(cmd);
  } else if (cmd == "ALARM" || cmd == "ALARM ON" || cmd == "CROSSING" ||
             cmd == "CROSSING ON" || cmd == "TEST_ALARM" || cmd == "ALARM ALL" ||
             cmd == "ALARM ALL ON" || cmd == "TEST_ALARM_ALL" || cmd == "ALARM OFF" ||
             cmd == "CROSSING OFF" || cmd == "STOP ALARM" || cmd == "STOP_ALARM" ||
             cmd == "STOP CROSSING" || cmd == "STOP_CROSSING" || cmd == "ALARM ALL OFF") {
    handleSerialAlarmCommand(cmd);
...
#if AUDIO_SENSOR_USES_HWCDC
void audioSensorUsbCdcEvent(void*, esp_event_base_t eventBase, int32_t eventId, void*) {
  if (eventBase != ARDUINO_HW_CDC_EVENTS || eventId != ARDUINO_HW_CDC_RX_EVENT) return;

  static char lineBuffer[SERIAL_SETUP_CMD_BUFFER + 1];
  static uint8_t lineLength = 0;

  while (HWCDCSerial.available() > 0) {
    const char c = (char)HWCDCSerial.read();
    if (c == '\r') continue;

    if (c == '\n') {
      if (lineLength > 0) {
        portENTER_CRITICAL(&usbCdcRxMux);
        if (!usbCdcPendingCommandReady) {
          memcpy(usbCdcPendingCommand, lineBuffer, lineLength);
          usbCdcPendingCommand[lineLength] = '\0';
          usbCdcPendingCommandLen = lineLength;
          usbCdcPendingCommandReady = true;
        }
        portEXIT_CRITICAL(&usbCdcRxMux);
      }
      lineLength = 0;
      continue;
    }

    if (lineLength < SERIAL_SETUP_CMD_BUFFER) {
      lineBuffer[lineLength++] = c;
    }
  }
}

void servicePendingUsbCdcCommand() {
  char command[SERIAL_SETUP_CMD_BUFFER + 1];
  uint8_t commandLength = 0;

  portENTER_CRITICAL(&usbCdcRxMux);
  if (usbCdcPendingCommandReady) {
    commandLength = usbCdcPendingCommandLen;
    memcpy(command, usbCdcPendingCommand, commandLength);
    usbCdcPendingCommandReady = false;
  }
  portEXIT_CRITICAL(&usbCdcRxMux);

  if (commandLength > 0) {
    command[commandLength] = '\0';
    processSerialCommandLine(String(command));
  }
}
#endif
...
void checkSerialForSetupCommand() {
  while (Serial.available() > 0) {
    ...
    if (c == '\n') {
      ...
      processSerialCommandLine(String(serialSetupCmd));
      serialSetupCmdLen = 0;
      continue;
    }
  ...
  if (serialSetupCmdLen > 0 && (millis() - serialSetupLastByteMs) > SERIAL_SETUP_CMD_TIMEOUT_MS) {
    serialSetupCmd[serialSetupCmdLen] = '\0';
    Serial.println("Serial command received without line ending; processing it.");
    processSerialCommandLine(String(serialSetupCmd));
    serialSetupCmdLen = 0;
  }
}
...
void updateFlashingLights() {
  ...
  if (allOutputsTestActive) {
    writeShiftRegister(lastFlash ? 0xFF : 0x00);
  } else if (cfg.lightPattern == LIGHT_PATTERN_STROBE) {
...
void setup() {
#if AUDIO_SENSOR_USES_HWCDC
  // Use the standard Arduino CDC initialization.  The application does its
  // own polling in loop(), matching the proven USB echo diagnostic.
  Serial.begin(115200);
#else
  Serial.begin(115200);
#endif
  ...
  delay(80);
  // The PCB routes both UART directions.  Use the DFPlayer return path so
  // command acknowledgements and status responses are available for testing.
