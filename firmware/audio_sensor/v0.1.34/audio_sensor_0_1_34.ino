...
uint32_t lastStationStateMs = 0;
constexpr uint32_t STATION_STATE_INTERVAL_MS = 10000;
...
void mqttCallback(char* topic, byte* payload, unsigned int length) {
  ...
  for (uint8_t i = 0; i < EXTERNAL_OUTPUT_COUNT; ++i) {
    ...
    if (looksOn(command)) jmriOutputState[i] = true;
    else if (looksOff(command)) jmriOutputState[i] = false;
    else return;
    refreshExternalOutputs();
    publishJmriObjectState(objectIndex);
    return;
  }
...
void loop() {
  ...
  if (WiFi.status() == WL_CONNECTED) {
    ...
    if (cfg.mqttEnabled) {
      connectMqttIfNeeded();
      mqttClient.loop();
      if (mqttClient.connected() && !stationStateTopic.isEmpty() &&
          millis() - lastStationStateMs >= STATION_STATE_INTERVAL_MS) publishStationState();
