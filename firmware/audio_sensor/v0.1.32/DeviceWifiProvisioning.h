...
inline void onWiFiEvent(WiFiEvent_t event, arduino_event_info_t) {
  switch (event) {
    case ARDUINO_EVENT_WPS_ER_SUCCESS:
      wpsState = WpsState::Success;
      break;
    case ARDUINO_EVENT_WPS_ER_FAILED:
      wpsState = WpsState::Failed;
      break;
    case ARDUINO_EVENT_WPS_ER_TIMEOUT:
      wpsState = WpsState::TimedOut;
      break;
    default:
      break;
  }
...
inline bool provision(char* ssid, size_t ssidSize, char* password, size_t passwordSize,
  ...
  while (wpsState == WpsState::Running &&
         millis() - pairingStarted < WPS_PAIRING_WINDOW_MS) {
    delay(25);
  }
  ...
  while (WiFi.status() != WL_CONNECTED &&
         millis() - connectStarted < WPS_CONNECT_TIMEOUT_MS) {
    delay(50);
  }
