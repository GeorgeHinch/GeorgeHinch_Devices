...
constexpr const char* MANAGED_USERNAME_PREFIX = "hm-device-";
constexpr const char* MANAGED_TOPIC_PREFIX = "hakomachi/devices/";

inline bool isManaged(const char* username, const char* topicRoot) {
  return username && topicRoot &&
         strncmp(username, MANAGED_USERNAME_PREFIX,
                 strlen(MANAGED_USERNAME_PREFIX)) == 0 &&
         strncmp(topicRoot, MANAGED_TOPIC_PREFIX,
                 strlen(MANAGED_TOPIC_PREFIX)) == 0;
}
