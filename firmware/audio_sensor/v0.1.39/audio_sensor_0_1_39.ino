...
bool crossingLightState[2] = {false, false};
...
bool effectiveOutputState(uint8_t index) {
  ...
  const bool localActive = index < 2 && crossingActive && crossingLightState[index];
...
void setCrossingLights(bool d1On, bool d2On) {
  crossingLightState[0] = d1On;
  crossingLightState[1] = d2On;
...
const char* jmriObjectState(uint8_t index) {
  switch (index) {
    case JMRI_CROSSING: return crossingActive ? "THROWN" : "CLOSED";
...
void publishMqttState() {
  ...
  if (crossingActive != lastPublishedCrossingState) {
    ...
    publishJmriObjectState(JMRI_CROSSING);
  }
...
void mqttCallback(char* topic, byte* payload, unsigned int length) {
  ...
  const String jmriCrossingTopic = buildJmriTopic("turnout", jmriObjects[JMRI_CROSSING].fullId);
  if (t == jmriCrossingTopic) {
    if (command == "THROWN") {
      startCrossing(false);
...
void serviceMqttEnrollment() {
  ...
  const char* capabilities[] = {"crossing", "lightsOnly", "volume", "sensor1", "sensor2"};
...
void startManualButtonAction() {
  ...
  startCrossing(cfg.manualButtonAction == 2);
...
void appendStationConfiguration(JsonDocument& doc) {
  ...
  JsonObject trigger = addSection(
      ...
      "Choose how detected trains start and stop the crossing sequence.", "two");
  ...
  JsonObject leds = addSection(
      "onboardLeds", "Onboard LEDs", "Configure the crossing-light animation.", "one");
  ...
  JsonObject player = addSection(
      ...
      "Configure the DFPlayer Mini audio source used by the crossing.", "two");
  JsonObject playerStatus = addField(player, "playerPresent", "Player hardware", "status");
  playerStatus["readOnly"] = true;
  playerStatus["trueLabel"] = "Detected";
  playerStatus["falseLabel"] = "Not detected";
  ...
  JsonObject button = addSection(
      ...
      "Configure the physical button used to run the crossing manually.", "two");
...
void publishStationState() {
  ...
  doc["crossingActive"] = crossingActive;
  ...
  addToggle("crossing", "Crossing active", "Crossing controls", "Crossing",
            "crossingActive", "Run the crossing lights and configured audio.", false, 10);
  addToggle("lightsOnly", "Crossing lights only", "Crossing controls", "Crossing",
            "lightsOnly", "Mute crossing audio while keeping the lights available.", false, 20);
  JsonObject volume = capabilities.add<JsonObject>();
  volume["name"] = "volume";
  volume["label"] = "Audio volume";
  volume["group"] = "Crossing controls";
  volume["subgroup"] = "Audio";
  volume["type"] = "range";
  volume["action"] = "set";
  volume["stateKey"] = "volume";
  volume["description"] = "Set the crossing audio output level.";
...
bool handleStationCommand(const String& topic, const char* payload) {
  ...
  if (action == "configure" && control == "settings") {
    ...
    if (cfg.playerMuted) stopMp3(); else if (crossingActive) startMp3();
    saveConfig();
  } else if (action == "set" && control == "crossing") {
    if (doc["value"] | false) startCrossing(false); else stopCrossing();
...
void connectMqttIfNeeded() {
  ...
  mqttClient.subscribe(buildJmriTopic("turnout", jmriObjects[JMRI_CROSSING].fullId).c_str());
  for (uint8_t i = 0; i < EXTERNAL_OUTPUT_COUNT; ++i) {
