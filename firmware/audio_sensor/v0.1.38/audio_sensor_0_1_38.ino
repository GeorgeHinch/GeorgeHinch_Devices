...
void appendStationConfiguration(JsonDocument& doc) {
  ...
  for (uint8_t index = 0; index < EXTERNAL_OUTPUT_COUNT; ++index) {
    ...
    const char* configuredName = jmriObjects[JMRI_OUTPUT_1 + index].name;
    JsonObject output = addField(outputs, key,
                                 configuredName[0] ? configuredName : fallbackLabel, "select");
