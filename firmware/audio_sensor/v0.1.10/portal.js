...
const PREVIEW_DEFAULTS = {
  deviceId: 'AUDIOSEN_123456789ABC', ip: '192.168.4.1', firmware: '0.1.10',
  ...
  sensorCount: 2, sensors: [
    { present: true, threshold: 175, baseline: 200, calibrated: true, calibrating: false },
    { present: true, threshold: 175, baseline: 200, calibrated: true, calibrating: false }
  ],
  hysteresis: 50, samplePeriod: 40, clearDelayMs: 800,
  lightPattern: 0, defaultLightOnly: false,
  playerPresent: true, track: 1, volume: 25,
  manualButtonEnabled: true, manualButtonRunSeconds: 30,
  otaState: 'Up to date (v0.1.10)'
};
...
function setSensorStatus(index, sensor) {
  ...
  if (!sensor.present) {
    ...
    status.className = 'status detected';
    calibration.textContent = `Clear baseline: ${sensor.baseline} mm`;
  } else {
    status.textContent = 'Needs calibration';
    status.className = 'status missing';
...
document.querySelector('#settings-form').addEventListener('submit', (event) => {
  ...
  const notice = document.querySelector('#notice');
  notice.textContent = event.submitter?.id === 'calibrate-sensors'
