...
const PREVIEW_DEFAULTS = {
  deviceId: 'AUDIOSEN_123456789ABC', ip: '192.168.4.1', firmware: '0.1.33',
  ...
  externalOutputs: [0, 0, 1, 1, 1, 1, 1, 1],
  otaState: 'Up to date (v0.1.33)'
};
...
function updateVolumeValue() { document.querySelector('#volume-value').value = document.querySelector('#volume').value; }
...
const externalOutputIds = Array.from({ length: 8 }, (_, index) => `SIGNAL_OUTPUT_1_${index + 1}`);

function setExternalOutputNames(config) {
  const friendlyNames = new Map(
    (config.jmriOutputNames || []).map((item) => [item.id, item.name]).filter((item) => item[0] && item[1])
  );
  externalOutputIds.forEach((id, index) => {
    document.querySelector(`#externalOutput${index + 1}-title`).textContent = friendlyNames.get(id) || id;
  });
}
...
function populate(config) {
  ...
  sensorSummary.textContent = `${count}/2`;
  sensorSummary.className = `status ${count === 2 ? 'detected' : count === 1 ? 'partial' : 'missing'}`;
  setValue('lightPattern', config.lightPattern);
  (config.externalOutputs || [0, 0, 1, 1, 1, 1, 1, 1]).forEach((assignment, index) => setValue(`externalOutput${index + 1}`, assignment));
  ...
  setChecked('playerEnabled', !config.playerMuted);
...
document.querySelector('#volume').addEventListener('input', updateVolumeValue);
