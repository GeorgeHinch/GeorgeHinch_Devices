const PREVIEW_DEFAULTS = {
  deviceId: 'MOTORCON_123456789ABC', ip: '192.168.4.1', firmware: '1.1.32', connected: true,
  linked: true, hold: false, stopLoss: true,
  motors: [
    { direction: 'arc', speed: 500, arcStart: 0, arcEnd: 512, arcActive: 'end', position: 0, positionKnown: true, moving: false },
    { direction: 'cw', speed: 500, arcStart: 0, arcEnd: 512, arcActive: 'end', position: 0, positionKnown: false, moving: false }
  ],
  ...
  mqttUser: 'layout-controller', jmriChannel: '/trains/', motorLedAssignments: [0, 0], otaState: 'Up to date (v1.1.32)'
};
...
function arcDialMarkup(motor) {
  const marker = (kind, value) => `<i class="arc-marker ${kind}" style="--angle:${arcAngle(value)}deg"></i>`;
  return `<div class="arc-ring">${marker('start', motor.arcStart)}${marker('end', motor.arcEnd)}${motor.positionKnown ? marker('current', motor.position) : ''}<span>S</span><span>E</span></div>`;
}
...
function refreshArcUi(index) {
  ...
  summary.querySelector(`[data-arc-status="${number}"]`).textContent = motor.moving ? 'Moving' : motor.positionKnown ? `At step ${motor.position}` : 'Position needs setup';
  summary.querySelector(`[data-arc-range="${number}"]`).textContent = `Start ${motor.arcStart} · End ${motor.arcEnd} · ${motor.arcActive === 'start' ? 'Start' : 'End'} is active`;
  setValue(`arcStart${number}`, motor.arcStart);
  setValue(`arcEnd${number}`, motor.arcEnd);
  setValue(`arcActive${number}`, motor.arcActive || 'end');
}
...
function populate(config) {
  ...
  config.motors.forEach((motor) => {
    motor.arcStart ??= 0; motor.arcEnd ??= 512; motor.arcActive ??= 'end';
...
function refreshArcDialog() {
  ...
  document.querySelector('#arc-dialog-dial').innerHTML = arcDialMarkup(motor);
  ...
  document.querySelector('#arc-dialog-position').textContent = motor.positionKnown ? motor.position : 'Unknown';
  document.querySelector('#arc-dialog-active').value = motor.arcActive || 'end';
  document.querySelector('#arc-dialog-warning').textContent = motor.positionKnown
    ? 'Jog to fine-tune the physical position, then mark either endpoint.'
    : 'Position was not safely retained. Jog to a known physical point and mark it as Start or End.';
  arcDialog.querySelectorAll('[data-move-endpoint]').forEach(button => { button.disabled = !motor.positionKnown || motor.moving; });
}
...
async function motorPreviewAction(action, value) {
  ...
  if (isPreviewMode()) {
    ...
    if (action === 'jog' || action === 'moveEndpoint') await new Promise(resolve => setTimeout(resolve, Math.abs(Number(value)) > 0 ? 450 : 850));
...
arcDialog.querySelectorAll('[data-jog]').forEach(button => button.addEventListener('click', () => motorPreviewAction('jog', button.dataset.jog).catch(error => alert(error.message))));
arcDialog.querySelectorAll('[data-mark-endpoint]').forEach(button => button.addEventListener('click', () => motorPreviewAction('markEndpoint', button.dataset.markEndpoint).catch(error => alert(error.message))));
arcDialog.querySelectorAll('[data-move-endpoint]').forEach(button => button.addEventListener('click', () => motorPreviewAction('moveEndpoint', button.dataset.moveEndpoint).catch(error => alert(error.message))));
document.querySelector('#arc-dialog-active').addEventListener('change', event => {
  const motor = currentConfig.motors[activeArcMotor];
  motor.arcActive = event.target.value;
  setValue(`arcActive${activeArcMotor + 1}`, motor.arcActive);
  refreshArcUi(activeArcMotor);
});
