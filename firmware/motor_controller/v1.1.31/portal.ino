...
void handleConfigJson() {
  ...
  for (uint8_t i = 0; i < MOTOR_COUNT; ++i) {
    ...
    motor["direction"] = !motors[i].enabled ? "disabled" : (motors[i].reverse ? "ccw" : "cw");
...
void handleConfigSave() {
  ...
  if (motors[0].enabled) motors[0].reverse = direction1 == "ccw";
  ...
  if (settings.linkMotorGroups) {
    ...
    if (motors[1].enabled) motors[1].reverse = direction2 == "ccw";
