...
void handleConfigJson() {
  ...
  JsonArray motorLedAssignments = doc["motorLedAssignments"].to<JsonArray>();
  for (uint8_t i = 0; i < MOTOR_COUNT; ++i) motorLedAssignments.add(settings.motorLedAssignment[i]);
  ...
  for (uint8_t i = 0; i < OBJECT_COUNT; ++i) {
    if (objects[i].kind == OBJ_MEMORY || !objectJmriEnabled(i) || !objects[i].name[0]) continue;
    JsonObject item = jmriOutputNames.add<JsonObject>();
    item["id"] = objects[i].localId;
  ...
  for (uint8_t i = 0; i < MOTOR_COUNT; ++i) {
    ...
    motor["direction"] = motorModeText(i);
    motor["speed"] = motors[i].speed;
    motor["arcStart"] = motors[i].arcStart;
    motor["arcEnd"] = motors[i].arcEnd;
    motor["arcPath"] = motors[i].arcClockwise ? "clockwise" : "counterclockwise";
    motor["position"] = motors[i].position;
    motor["positionKnown"] = motors[i].positionKnown;
    motor["moving"] = motorMoving(i);
  }
  ...
  for (uint8_t i = 0; i < SENSOR_COUNT; ++i) {
    ...
    sensor["baseline"] = ranges[i].baselineMm;
    sensor["distance"] = ranges[i].distanceMm;
    sensor["calibrated"] = ranges[i].calibrated;
    sensor["calibrating"] = ranges[i].calibrating;
  }
...
void handleManifestConfigure() {
  JsonDocument doc;
  if (deserializeJson(doc, configServer.arg("plain")) || !doc["values"].is<JsonObject>()) {
    configServer.send(400, "application/json", "{\"ok\":false,\"error\":\"Invalid configuration\"}");
    return;
  }
...
void handleManifestCommand() {
  JsonDocument doc;
  if (deserializeJson(doc, configServer.arg("plain"))) {
    configServer.send(400, "application/json", "{\"ok\":false,\"error\":\"Invalid command\"}");
    return;
  }
...
void handleConfigSave() {
  ...
  const String direction1 = configServer.arg("dir1");
  motors[0].enabled = direction1 != "disabled";
  motors[0].positionedArc = direction1 == "arc";
  if (motors[0].enabled && !motors[0].positionedArc) motors[0].reverse = direction1 == "ccw";
  motors[0].speed = boundedFormNumber("speed1", motors[0].speed,
                                      MIN_SPEED_STEPS_PER_SECOND, MAX_SPEED_STEPS_PER_SECOND);
  motors[0].arcStart = configServer.arg("arcStart1").toInt();
  motors[0].arcEnd = configServer.arg("arcEnd1").toInt();
  motors[0].arcClockwise = configServer.arg("arcPath1") != "counterclockwise";
  if (settings.linkMotorGroups) {
    ...
    const String direction2 = configServer.arg("dir2");
    motors[1].enabled = direction2 != "disabled";
    motors[1].positionedArc = direction2 == "arc";
    if (motors[1].enabled && !motors[1].positionedArc) motors[1].reverse = direction2 == "ccw";
    motors[1].speed = boundedFormNumber("speed2", motors[1].speed,
                                        MIN_SPEED_STEPS_PER_SECOND, MAX_SPEED_STEPS_PER_SECOND);
    motors[1].arcStart = configServer.arg("arcStart2").toInt();
    motors[1].arcEnd = configServer.arg("arcEnd2").toInt();
    motors[1].arcClockwise = configServer.arg("arcPath2") != "counterclockwise";
  }

  settings.holdWhenStopped = configServer.hasArg("hold");
  settings.stopOnConnectionLoss = configServer.hasArg("stoploss");
  for (uint8_t i = 0; i < MOTOR_COUNT; ++i) {
    String key = String("motorLed") + String(i + 1) + String("Assignment");
    settings.motorLedAssignment[i] = static_cast<uint8_t>(boundedFormNumber(
        key.c_str(), settings.motorLedAssignment[i], OUTPUT_RESERVED, OUTPUT_SHARED));
  }
  ...
  if (configServer.hasArg("sensorTriggerEnabled")) {
    settings.sensorControlMode = static_cast<SensorControlMode>(boundedFormNumber(
        "sensormode", SENSOR_CONTROL_ANY_TIMED, SENSOR_CONTROL_ANY_TIMED, SENSOR_CONTROL_ENTER_EXIT));
  } else {
...
void handleConfigCalibrate() {
  const int requestedSensor = configServer.arg("sensor").toInt() - 1;
  if (requestedSensor >= 0 && requestedSensor < SENSOR_COUNT) {
    startRangeCalibration((uint8_t)requestedSensor);
  }
...
void handleMotorCommand() {
  const int motorNumber = configServer.arg("motor").toInt();
  if (motorNumber < 1 || motorNumber > MOTOR_COUNT) {
    configServer.send(400, "text/plain", "Invalid motor group");
    return;
  }
  const uint8_t index = static_cast<uint8_t>(motorNumber - 1);
  const String action = configServer.arg("action");
  const String value = configServer.arg("value");
  bool accepted = true;
  if (action == "jog") {
    const int32_t steps = constrain(value.toInt(), -50, 50);
    if (steps == 0) accepted = false;
    else startFiniteMove(index, steps);
  } else if (action == "home") {
    accepted = moveMotorHome(index);
  } else if (action == "markEndpoint" && (value == "start" || value == "end")) {
    markMotorEndpoint(index, value == "end");
  } else if (action == "moveEndpoint" && (value == "start" || value == "end")) {
    accepted = moveMotorToEndpoint(index, value == "end");
  } else {
    accepted = false;
  }
  if (accepted) publishStationState();
  configServer.sendHeader("Cache-Control", "no-store");
  configServer.send(accepted ? 200 : 409, "application/json", accepted ? "{\"ok\":true}" : "{\"ok\":false,\"error\":\"Position needs setup\"}");
}
...
void initializeConfigPortal() {
  ...
  configServer.on("/api/configure", HTTP_POST, handleManifestConfigure);
  configServer.on("/api/command", HTTP_POST, handleManifestCommand);
  configServer.on("/save", HTTP_POST, handleConfigSave);
  configServer.on("/calibrate", HTTP_POST, handleConfigCalibrate);
  configServer.on("/motor", HTTP_POST, handleMotorCommand);
