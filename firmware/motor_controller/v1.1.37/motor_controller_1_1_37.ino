...
#define FIRMWARE_VERSION "1.1.37"
...
int32_t normalizeArcPosition(int32_t value) {
  int32_t normalized = value % 4096;
  return normalized < 0 ? normalized + 4096 : normalized;
}
...
bool motorAtActiveEndpoint(uint8_t index) {
  ...
  return normalizeArcPosition(motor.position) == normalizeArcPosition(motor.arcEnd);
}
...
bool operationalMotorLedState(uint8_t index) {
...
void setMotorLed(uint8_t index) {
  indicatorOutputs.set(index, operationalMotorLedState(index));
}
...
void setMotorLedTest(uint8_t index, bool enabled) {
  ...
  indicatorOutputs.test(index, enabled);
}

bool serviceMotorLedTest(uint32_t now) {
  return indicatorOutputs.consumeEnded();
}
...
void saveMotorSettings(uint8_t index) {
  ...
  snprintf(key, sizeof(key), "arcCw%u", index);
  preferences.putBool(key, motors[index].arcClockwise);
  ...
  if (settings.linkMotorGroups && index == 0) {
    ...
    motors[1].arcClockwise = motors[0].arcClockwise;
    ...
    preferences.putBool("arcCw1", motors[1].arcClockwise);
...
bool moveMotorHome(uint8_t index) {
  MotorState& motor = motors[index];
  if (!motor.positionKnown) return false;
  const int32_t current = normalizeArcPosition(motor.position);
  const int32_t clockwiseSteps = (4096 - current) % 4096;
  const int32_t shortestSteps =
      clockwiseSteps <= 2048 ? clockwiseSteps : clockwiseSteps - 4096;
  startFiniteMove(index, shortestSteps);
  return true;
}
...
bool moveMotorToEndpoint(uint8_t index, bool endEndpoint) {
  ...
  if (!motor.positionedArc || !motor.positionKnown) return false;
  const int32_t current = normalizeArcPosition(motor.position);
  const int32_t target = normalizeArcPosition(endEndpoint ? motor.arcEnd : motor.arcStart);
  const int32_t clockwiseSteps = (target - current + 4096) % 4096;
  const bool moveClockwise = endEndpoint ? motor.arcClockwise : !motor.arcClockwise;
  const int32_t routeSteps = moveClockwise
      ? clockwiseSteps
      : (clockwiseSteps == 0 ? 0 : clockwiseSteps - 4096);
  startFiniteMove(index, routeSteps);
  return true;
}
...
void setConfiguredRun(uint8_t index, bool active, bool publish) {
  ...
  if (motor.positionedArc) {
    moveMotorToEndpoint(index, active);
    return;
  }
...
void loadSettings() {
  ...
  for (uint8_t i = 0; i < MOTOR_COUNT; ++i) {
    ...
    snprintf(key, sizeof(key), "arcCw%u", i);
    motors[i].arcClockwise = preferences.getBool(key, true);
...
void saveAllSettings() {
  ...
  for (uint8_t i = 0; i < MOTOR_COUNT; ++i) {
    ...
    snprintf(key, sizeof(key), "arcCw%u", i);
    preferences.putBool(key, motors[i].arcClockwise);
    snprintf(key, sizeof(key), "arcStart%u", i);
...
void objectStateText(uint8_t objectIndex, char* output, size_t outputSize) {
  ...
  if (objectIndex == O_M1_LED || objectIndex == O_M2_LED) {
    ...
    snprintf(output, outputSize, "%s", operationalMotorLedState(index) ? "ON" : "OFF");
    return;
  }
...
void appendStationConfiguration(JsonDocument& doc, bool includeLiveOutputs) {
  ...
  for (uint8_t index = 0; index < MOTOR_COUNT; ++index) {
    ...
    values[prefix + "ArcPath"] = motors[index].arcClockwise ? "clockwise" : "counterclockwise";
  ...
  const auto indicators = indicatorOutputs.snapshot();
  values["motorLed1Testing"] = indicators.testIndex == 0;
  values["motorLed2Testing"] = indicators.testIndex == 1;
  values["begLedTesting"] = indicators.testIndex == 2;
  values["begLedAssignment"] = 0;
  values["begLedActive"] = begLedState;
  for (uint8_t index = 0; index < MOTOR_COUNT; ++index) {
    const String key = String("motorLed") + String(index + 1);
    values[key + "Active"] = operationalMotorLedState(index);
    if (includeLiveOutputs) {
      const bool local = motorMoving(index) && settings.motorLedAssignment[index] != OUTPUT_JMRI;
      const bool remote = jmriMotorLedState[index] && settings.motorLedAssignment[index] != OUTPUT_RESERVED;
      const char* source = local && remote ? "Motor activity + JMRI" : local ? "Motor activity" :
          remote ? "JMRI" : settings.motorLedAssignment[index] == OUTPUT_RESERVED ? "Motor activity (idle)" :
          settings.motorLedAssignment[index] == OUTPUT_JMRI ? "JMRI (idle)" : "Shared (idle)";
      indicatorOutputs.appendLive(values, key, index, source);
    }
  }
  if (includeLiveOutputs) indicatorOutputs.appendLive(values, "begLed", 2,
      settings.globalBegLedsEnabled ? "Ready indicator" : "Global JMRI override");
  ...
  arc1["pathKey"] = "motor1ArcPath"; arc1["positionKey"] = "motor1Position";
  arc1["knownKey"] = "motor1PositionKnown"; arc1["movingKey"] = "motor1Moving";
  arc1["homeAction"] = "home"; arc1["homeLabel"] = "Home";
  ...
  arc2["pathKey"] = "motor2ArcPath"; arc2["positionKey"] = "motor2Position";
  arc2["knownKey"] = "motor2PositionKnown"; arc2["movingKey"] = "motor2Moving";
  arc2["homeAction"] = "home"; arc2["homeLabel"] = "Home";
  ...
  JsonObject leds = addSection(
      ...
      "Choose whether each motor-status LED is reserved for the board, controlled by JMRI, or shared.", "one");
  const char* ledKeys[] = {"motorLed1Assignment", "motorLed2Assignment"};
  for (uint8_t index = 0; index < MOTOR_COUNT; ++index) {
    const uint8_t objectIndex = index == 0 ? O_M1_LED : O_M2_LED;
    JsonObject led = addField(leds, ledKeys[index],
                              index == 0 ? "Motor group 1 status LED" : "Motor group 2 status LED",
                              "output");
    ...
    LocalIndicatorOutputs::appendFields(leds, String("motorLed") + String(index + 1), "Live output status");
  }

  JsonObject readyLed = addField(leds, "begLedAssignment", "Ready indicator", "output");
  readyLed["readOnly"] = true;
  readyLed["resource"] = objects[O_BEG].fullId;
  readyLed["testAction"] = "test";
  readyLed["testControl"] = "begLed";
  readyLed["testStateKey"] = "begLedTesting";
  readyLed["testTimeoutSeconds"] = 30;
  addNumberOption(readyLed, 0, "Managed ready indicator");
  LocalIndicatorOutputs::appendFields(leds, "begLed", "Ready indicator live status");
...
bool applyStationConfiguration(JsonObject values) {
  ...
  for (uint8_t index = 0; index < MOTOR_COUNT; ++index) {
    ...
    if (!values[prefix + "ArcPath"].isNull()) {
      motors[index].arcClockwise = values[prefix + "ArcPath"].as<String>() != "counterclockwise";
    }
  }
  if (settings.linkMotorGroups) {
    motors[1].arcClockwise = motors[0].arcClockwise;
...
bool executeStationCommand(JsonDocument& doc) {
  ...
  if (action == "configure" && control == "settings") {
  ...
  } else if (action == "home" && control.startsWith("motor")) {
    const int motorNumber = control.substring(5).toInt();
    if (motorNumber >= 1 && motorNumber <= MOTOR_COUNT) moveMotorHome(motorNumber - 1);
    ...
    indicatorOutputs.test(2, doc["value"] | true);
...
void handleButton() {
  ...
  if (settings.begButtonAction != BEG_BUTTON_DISABLED) publishStationState();
}
...
void setupHardware() {
  ...
  const uint8_t indicatorPins[] = {PIN_MOTOR1_LED, PIN_MOTOR2_LED, PIN_BEG_LED};
  if (!indicatorOutputs.begin(indicatorPins, 3)) Serial.println("Indicator timing unavailable; LEDs held off.");
  refreshIndicatorLeds(false);
}
