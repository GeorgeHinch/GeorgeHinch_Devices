...
constexpr wifi_power_t STATION_TX_POWER = WIFI_POWER_8_5dBm;

inline bool applyStationTxPower() {
  return WiFi.setTxPower(STATION_TX_POWER);
}
...
inline void configureStation(const char* deviceName) {
  ...
  WiFi.mode(WIFI_STA);
  Serial.printf("Wi-Fi station TX power: 8.5 dBm (%s).\n",
                applyStationTxPower() ? "applied" : "failed");
}
