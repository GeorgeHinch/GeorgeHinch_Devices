...
#define FIRMWARE_VERSION "1.1.30"
...
void appendStationConfiguration(JsonDocument& doc) {
  ...
  values["motor1Run"] = motors[0].continuous;
  values["motor2Run"] = motors[1].continuous;
  values["linkMotorGroups"] = settings.linkMotorGroups;
  values["motor1Direction"] = !motors[0].enabled ? "disabled" : (motors[0].reverse ? "ccw" : "cw");
  values["motor1Speed"] = motors[0].speed;
  values["motor2Direction"] = !motors[1].enabled ? "disabled" : (motors[1].reverse ? "ccw" : "cw");
  ...
  JsonObject groups = addSection(
      ...
      "Linked groups share Motor Group 1's saved direction and speed. Disabled groups are skipped by sensor automation.",
      "two");
  ...
  const char* ledLabels[] = {"Motor group 1 LED", "Motor group 2 LED"};
  for (uint8_t index = 0; index < MOTOR_COUNT; ++index) {
    JsonObject led = addField(leds, ledKeys[index], ledLabels[index], "select");
...
bool applyStationConfiguration(JsonObject values) {
  ...
  if (!values["motor1Direction"].isNull()) {
    ...
    if (motors[0].enabled) motors[0].reverse = direction == "ccw";
  }
  ...
  if (settings.linkMotorGroups) {
    ...
    if (!values["motor2Direction"].isNull()) {
      ...
      if (motors[1].enabled) motors[1].reverse = direction == "ccw";
    }
...
void publishStationState() {
  ...
  doc["motor1Run"] = motors[0].continuous;
  doc["motor1Direction"] = motors[0].reverse;
  doc["motor1Speed"] = motors[0].speed;
  doc["motor2Run"] = motors[1].continuous;
  ...
  if (action == "configure" && control == "settings") {
    ...
    for (uint8_t i = 0; i < MOTOR_COUNT; ++i) {
      ...
      if (control == prefix + "Run") {
        if (enabled) startContinuous(i); else stopMotor(i);
