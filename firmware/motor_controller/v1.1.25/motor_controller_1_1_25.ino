...
void mqttCallback(char* topicText, byte* payload, unsigned int length) {
  ...
  for (uint8_t i = 0; i < OBJECT_COUNT; ++i) {
    ...
    if (objects[i].kind == OBJ_TURNOUT) handleTurnoutCommand(i, message.c_str());
      ...
      if (message == "ON") jmriMotorLedState[ledIndex] = true;
      else if (message == "OFF") jmriMotorLedState[ledIndex] = false;
      else return;
      refreshIndicatorLeds(false);
      publishBooleanObject(i, effectiveMotorLedState(ledIndex));
    }
    return;
  }
}
