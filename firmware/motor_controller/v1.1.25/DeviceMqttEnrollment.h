...
inline bool copyChecked(char* destination, size_t destinationSize, const char* value) {
  if (!destination || destinationSize == 0 || !value || !value[0] ||
      strlen(value) >= destinationSize) {
    return false;
  }
...
inline bool enroll(const char* deviceId, const char* displayName,
                   ...
                   size_t capabilityCount, char* broker, size_t brokerSize,
                   uint16_t& port, char* username, size_t usernameSize,
  ...
  if (status == 423) {
    Serial.println(F("HakoMachi MQTT enrollment is waiting for an open pairing window."));
    return false;
  }
  if (status != HTTP_CODE_CREATED) {
    Serial.printf("HakoMachi MQTT enrollment unavailable (HTTP %d).\n", status);
    return false;
  }

  JsonDocument response;
  if (deserializeJson(response, responsePayload)) {
    Serial.println(F("HakoMachi MQTT enrollment returned invalid JSON."));
    return false;
  }
  ...
  if (strcmp(responseDeviceId, deviceId) != 0 || responsePort < 1 ||
    ...
    Serial.println(F("HakoMachi MQTT enrollment response failed validation."));
    return false;
  }
  ...
  Serial.printf("HakoMachi MQTT enrollment complete: %s:%u (%s)\n", broker, port,
                topicRoot);
  return true;
}
