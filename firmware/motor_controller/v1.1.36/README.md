...
Station-mode Wi-Fi is capped at 8.5 dBm. Hardware validation showed that this
motor carrier completes association, DHCP, and application traffic at 8.5 dBm,
while maximum ESP32-C3 transmit power destabilizes its station transmissions.
The cap is reapplied before provisioning, initial connection, and reconnects.
Enrollment is serviced immediately after Wi-Fi connects, before requiring saved
MQTT broker settings, so a factory-new controller can receive those settings
from the station.
Managed credentials are recognized by their full prefix lengths so subsequent
boots reuse the saved account instead of enrolling and rotating it again.
...
mirrors Group 1's mode, speed, and positioned arc and its controls are disabled. MQTT
